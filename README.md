...
- based on `ubuntu 18.04 (boinic)` image

using:

- java oracle jdk8u181
- jce policy
...
- [`daggerok/kafka:latest` based on `ubuntu`, `oraclejdk8` and `spring-cloud-cli`](https://github.com/daggerok/kafka/blob/master/Dockerfile)
...
#docker run --rm --name run-my-kafka -p 2181:2181 -p 9092:9092 daggerok/kafka
